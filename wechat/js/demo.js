...
canvas.width = 375//写死，适配手机屏幕
canvas.height = 667//写死，适配手机屏幕
...
function generate(cxt) {
	...
	var chatTime = new Array(7)

    // 气泡到边框的上边距
    var dis = 0
	
    for (var i = 1; i < 8; i++) {//最多七个
		// 获得 7 组共 28 个 html 的标签对象
		...
		//得到id为time1~time7的输入框对象
		chatTime[i] = document.getElementById('chatTime' + i)
		...
		if (i == 1) {
			...
			dis = dis + 20 + 40  //时间气泡的高度   
        }

        // 如果复选框被勾了
        if (chk[i].checked) {
            if(null != chatTime[i].value && '' != chatTime[i].value){//填写了时间
				drawChatTime(cxt, chatTime[i].value, dis)
				dis = dis + 20 //加上时间高度
			}
			// 得到下拉选项框的选项索引值
			// 0：我，1：对方
...
根据文本框的时间生成聊天时间
*/
function drawChatTime(cxt, timeVal, y){
	cxt.beginPath()
    cxt.font = 'lighter 10px STHeitiSC-Light'
    // 文本水平居中
    cxt.textAlign = 'center'
    cxt.textBaseline = 'top'
    cxt.fillStyle = 'black'
    cxt.fillText(timeVal, 187, y)
}
