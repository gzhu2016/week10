...
var DD_belatedPNG = {
	...
	createVmlStyleSheet: function () {
		...
		if (screenStyleSheet.styleSheet) {
			...
			screenStyleSheet.addRule('img.' + this.ns + '_sizeFinder', 'behavior:none; border:none; ' +
				'position:absolute; z-index:-1; top:-10000px; visibility:hidden;');
	...
	attachHandlers: function (el) {
		...
		if (el.nodeName == 'A') {
			moreForAs = {mouseleave: 'handlePseudoHover', mouseenter: 'handlePseudoHover',
				focus: 'handlePseudoHover', blur: 'handlePseudoHover'};
			for (a in moreForAs) {			
	...
	vmlFill: function (el) {
		...
		if (elStyle.backgroundImage != 'none' || el.isImg) {
			...
			if (!lib.imgSize[el.vmlBg]) {
				...
				img.runtimeStyle.cssText = 'behavior:none; position:absolute; left:-10000px;' +
					' top:-10000px; border:none; margin:0; padding:0;';
	...
	vmlOffsets: function (el) {
		...
		size = {'W':el.clientWidth+1, 'H':el.clientHeight+1, 'w':this.imgSize[el.vmlBg].width,
			'h':this.imgSize[el.vmlBg].height, 'L':el.offsetLeft, 'T':el.offsetTop,
			'bLW':el.clientLeft, 'bTW':el.clientTop};
