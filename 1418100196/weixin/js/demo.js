...
function drawMyMessage(cxt, msg, y) {
    ...
    if (msgLength <= 230) {
        ...
        drawMyChat(cxt, 375 - 40 - 10 - 4 - 230 - 24, y, 230 + 28, 40 + Math.ceil(msgLength / 230) * (5 + 16), 5)
        ...
        cxt.fillStyle = 'black'
        // 气泡内边距为12，所以文本的x和y 要在气泡的基础上+12
        var line = Math.ceil(msgLength / 230)
        for (var i = 0, j = 0; i < line; i++) {

            if (j + 15 > msg.length) {
                cxt.fillText(msg.substring(j, msg.length), 375 - 40 - 10 - 4 - 230 - 24 + 12, (y + 12) + 21 * i)
            }
            if (j + 15 <= msg.length) {
                cxt.fillText(msg.substring(j, j + 15), 375 - 40 - 10 - 4 - 230 - 24 + 12, (y + 12) + 21 * i)
                j = j + 15;
            }
        }
    }
...
function drawHisMessage(cxt, msg, y) {
    ...
    if (msgLength <= 230){
        ...
        drawHisChat(cxt, 40 + 10 +4, y, 230 + 28, 40 + Math.ceil(msgLength / 230) * (5 + 16), 5)
        cxt.textBaseline = 'top'
        cxt.fillStyle = 'black'
        var line = Math.ceil(msgLength / 230)
        for (var i = 0, j = 0; i < line; i++) {
            ...
            if (j + 15 > msg.length) {
                cxt.fillText(msg.substring(j, msg.length), 40 + 10 + 4 +12, (y + 12) + 21 * i)
            }
            if (j + 15 <= msg.length) {
                cxt.fillText(msg.substring(j, j + 15), 40 + 10 +4 + 12, (y + 12) + 21 * i)
                j = j + 15;
            }
        }
    }
